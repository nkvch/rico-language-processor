...
Now, based on what user said, return JSON. If user asked a question that is not directly related to any of parameters of current task - you set "unexpected_question" to string containing information about that question (e .g. if question was "Should I give you big or small cup?" then "unexpected_question" is "big_or_small_cup"), otherwise - to null. If you set "unexpected_question" to some string that means that you should set "name" to null. If user input sentence is not a question and cannot be associated in any way with any of given intents or you don't understand the user intent - you just return "null". You cannot set "unexpected_question" to not null if what user said is not a question.
